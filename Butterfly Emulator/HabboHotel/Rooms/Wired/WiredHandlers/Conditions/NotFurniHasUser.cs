...
{
    class NotFurniHasUsers : IWiredCondition
    {
        private RoomItem item;
        private List<RoomItem> items;
        private bool isDisposed;

        public NotFurniHasUsers(RoomItem item, List<RoomItem> items)
        {
            this.item = item;
            this.items = items;
        ...
        public bool AllowsExecution(RoomUser user)
        {
            ...
            return !items.Any(item => item.GetRoom().GetGameMap().GetUsersOnItem(item).Count > 0 ); // aqui ele vai pegar todo o tamanho do mobi e não só 1 quadrado (tipo tapetes e tals)
            //return !items.Any(item => item.GetRoom().GetGameMap().GetRoomUsers(item.Coordinate).Count > 0);
