...
namespace Butterfly.HabboHotel.Items
{
    ...
    public class RoomItem : IEquatable<RoomItem>
    {
        ...
        internal Point RightSideTwoTiles
        {
            get
            {
                if (Rot == 0)
                    return new Point(mX + 2, mY);
                else if (Rot == 2)
                    return new Point(mX, mY + 2);
                else if (Rot == 4)
                    return new Point(mX - 2, mY);
                else if (Rot == 6)
                    return new Point(mX, mY - 2);
        ...
        internal Point LeftSideTwoTiles
        {
            get
            {
                if (Rot == 0)
        ...
        internal Point SquareBehindTwoTiles
        {
            get
            {
                var Sq = new Point(mX, mY);
            ...
            get
            {
                ...
                switch (GetBaseItem().InteractionType)
                {
                    ...
                    case InteractionType.fbgate:
                        return new InteractorFootballGate();
                    default:
        ...
        internal void Serialize(ServerMessage Message)
        {
            ...
            if (IsFloorItem)
            {
                ...
                Message.AppendInt32(-1);
                Message.AppendInt32(GetBaseItem().Modes > 1 ? 1 : 0/* || GetBaseItem().InteractionType != InteractionType.none ? 1 : 0*/); // Botón Usar.
