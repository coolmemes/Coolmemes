    ...
    class Bully
    {
        ...
        internal void OnCycle()
        {
            ...
            else if(this.bullyState == BullyState.FINISHED)
            {
                switch (this.bullySolution)
                {
                    ...
                    /*case BullySolution.HORROR:
                        {
                            GameClient TargetClient = HabboEnvironment.GetGame().GetClientManager().GetClientByUserID(ReportedId);
                            Habbo habbo = UsersCache.getHabboCache(ReportedId);

                            if (habbo != null && habbo.Rank <= 1)
                                //HabboEnvironment.GetGame().GetBanManager().BanUser(TargetClient, habbo.Username, "", "Baneado por los HabboAlfas.", 21600, null);
                            break;
                        }*/
                }
                ...
                if (guardianClient != null && guardianClient.GetHabbo() != null)
                {
                    ...
                    // HabboEnvironment.GetGame().GetAlfaManager().GetBullyManager().RemoveGuardian(GuardianId);
                    // HabboEnvironment.GetGame().GetAlfaManager().GetBullyManager().AddGuardian(GuardianId);
                }
        ...
        internal bool SearchGuardian()
        {
            ...
            foreach (UInt32 guardianId in onlineGuardians)
            {
                ...
                {
                    //HabboEnvironment.GetGame().GetAlfaManager().GetBullyManager().RemoveGuardian(guardian.Key);
                    continue;
                }
